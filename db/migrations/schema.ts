import { pgTable, serial, varchar, text, date, foreignKey, unique, integer, vector, primaryKey } from "drizzle-orm/pg-core"
...
export const dettagliNutrizionaliGiornalieri = pgTable("dettagli_nutrizionali_giornalieri", {
	id: serial().primaryKey().notNull(),
	...
	unique("dettagli_nutrizionali_giornalieri_piano_id_giorno_settimana_key").on(table.pianoId, table.giornoSettimana),
]);
...
export const pianiPasti = pgTable("piani_pasti", {
	pianoId: integer("piano_id").notNull(),
	...
	primaryKey({ columns: [table.pianoId, table.pastoId, table.giornoSettimana], name: "piani_pasti_pkey"}),
]);
